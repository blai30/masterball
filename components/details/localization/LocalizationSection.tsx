...
export default async function LocalizationSection({ species }: { species: PokemonSpecies }) {
      ...
      <ul className="grid max-w-lg grid-flow-col grid-cols-1 grid-rows-11 gap-2 @md:grid-cols-2 @md:grid-rows-6">
        {species.names
          ...
          .map((name) => {
            ...
            return (
              <li key={language.name} className="flex w-full flex-col items-center">
                <p className="flex w-full items-center gap-3">
                  <span title={language.name} className="w-1/3 text-right font-medium">
                    {language.name}
                  </span>
                  <span className="text-zinc-700 dark:text-zinc-300">{localizedName}</span>
                </p>
              </li>
            )
          })}
