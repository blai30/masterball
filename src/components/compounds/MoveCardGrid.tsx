...
import FilterBar, { type FilterConfig, type FilterOption } from '@/components/shared/FilterBar'
...
export default function MoveCardGrid({
  ...
  const damageClassFilters: FilterOption[] = useMemo(
    () => Object.entries(DamageClassKey).map(([key, value]) => ({ label: key, value })),
    []
  )

  const [search, setSearch] = useState(() => getSearchParam('q') ?? '')
  const [sortKey, setSortKey] = useState(() => getSearchParam('sort') ?? DEFAULT_SORT_KEY)
  const [sortDirection, setSortDirection] = useState<SortDirection>(
    () => (getSearchParam('dir') as SortDirection) ?? DEFAULT_SORT_DIRECTION
  )
  const [typeFilter, setTypeFilter] = useState<string[]>(
    () => getSearchParam('type')?.split(',').filter(Boolean) ?? []
  )
  const [damageClassFilter, setDamageClassFilter] = useState<string[]>(
    () => getSearchParam('class')?.split(',').filter(Boolean) ?? []
  )
  ...
  useEffect(() => {
    syncUrlParams({ search, sortKey, sortDirection, typeFilter, damageClassFilter, currentPage })
  }, [search, sortKey, sortDirection, typeFilter, damageClassFilter, currentPage, syncUrlParams])

  const handleSearchChange = useCallback((value: string) => { setSearch(value); setCurrentPage(DEFAULT_PAGE) }, [])
  const handleSortKeyChange = useCallback((key: string) => { setSortKey(key); setCurrentPage(DEFAULT_PAGE) }, [])
  const handleSortDirectionChange = useCallback((dir: SortDirection) => { setSortDirection(dir); setCurrentPage(DEFAULT_PAGE) }, [])
  const handleTypeFilterChange = useCallback((values: string[]) => { setTypeFilter(values); setCurrentPage(DEFAULT_PAGE) }, [])
  const handleDamageClassFilterChange = useCallback((values: string[]) => { setDamageClassFilter(values); setCurrentPage(DEFAULT_PAGE) }, [])
  ...
  const filters: FilterConfig[] = useMemo(
    ...
    [typeFilter, typeFilters, handleTypeFilterChange, damageClassFilter, damageClassFilters, handleDamageClassFilterChange]
  )
  ...
  const filteredData = useMemo(() => {
    ...
    filtered = filtered.filter((resource) => {
      ...
      const classMatch = damageClassFilter.length === 0 || damageClassFilter.includes(resource.damageClass)
      return typeMatch && classMatch
    })
    if (search) {
      const fuse = new Fuse(filtered, { keys: ['name'], threshold: 0.4, ignoreLocation: false })
    ...
    return filtered
  }, [data, typeFilter, damageClassFilter, search, sortKey, sortDirection, versionGroup, filterByVersionGroup])
