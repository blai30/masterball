...
function EvolutionNode({ node, depth = 0 }: { node: EvolutionNodeData; depth?: number }) {
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center gap-2">
        ...
        <div className="flex flex-col gap-1">
          <MonsterPill species={node.species} pokemon={node.pokemon} />
        </div>
      </div>
      ...
      {node.evolvesTo.length > 0 && (
        <div className="flex flex-col gap-2">
          {node.evolvesTo.map((child) => (
            <EvolutionNode key={child.species.name} node={child} depth={depth + 1} />
          ))}
        </div>
      )}
    </div>
...
export default function EvolutionSection({ evolutionRoot }: { evolutionRoot: EvolutionNodeData }) {
  return (
    <section className="flex flex-col gap-4 rounded-xl p-4 inset-ring-1 inset-ring-zinc-200 dark:inset-ring-zinc-800">
      ...
      <EvolutionNode node={evolutionRoot} />
    </section>
  )
}
